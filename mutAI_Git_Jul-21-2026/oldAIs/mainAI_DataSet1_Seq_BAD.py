...
class TestNN(nn.Module):
    def __init__(self):
        ...
        self.fc4 = nn.Linear(2000,10000)
        self.fc5 = nn.Linear(10000,2000)
...
#The following takes the ln of the inputs and outputs
...
default_dir = "mutAI/dataOutputFiles/outData_mut_size_DataSet1_Seq.txt"
...
def visuals(targ,guess,gsize,targ2,guess2,gsize2):
    ...
    if len(gsize2)>0:
        minGsize = min( min(gsize),min(gsize2) )

        maxGsize = max( max(gsize),max(gsize2) )

        lineP = [min(min(guess2),min(guess)),max(max(guess2),max(guess))]
        linePX= [min(min(targ2),min(targ)),max(max(targ2),max(targ))]
        for x in range(len(targ)):
            av=(targ2[x]+guess2[x])/2
            plt.plot([targ2[x],av],[guess2[x],av],lw=1,zorder=-1000000000000000,alpha=0.1,c="#000000")
    ...
    plt.scatter(targ,guess,c=gsize,cmap='viridis',vmin=minGsize,vmax=maxGsize,alpha=1,s=uhhWhat)
    
    if len(gsize2)>0: 
        plt.scatter(targ2,guess2,c=gsize2,cmap='viridis',vmin=minGsize,vmax=maxGsize,alpha=1,s=75)
    ...
    plt.savefig('mutAI/dataVisuals/DataSet1_Seq/plot_mut_Size_ln.png')
