...
def getData():
    ...
    inputs_Edt=undoScale(inputs.clone())
    for x in targets:
        ...
        rtn[4].append(inputs_Edt[len(rtn[4])][0].item())
        for y in range(1,5):
            
            rtn[4+y].append(inputs_Edt[len(rtn[4+y])][y].item()/2)
        count+=1
    return rtn

def undoScale(inRTNog):
    inRTN=inRTNog.clone()
    inRTN*=input_stds
    inRTN+=input_means
    return inRTN
...
def writeOutput(inRTN,file_name=default_dir,wrt="w"):
    ...
    with open(file_name,wrt) as f:
        ...
        header = ["Name","Mutation Rate*","Predicted Rate*","Error","Genome Size (KB)*","A%","G%","C%","T%","Data Input Step"]
        f.write("\n|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~|")
        f.write(f"\n|{header[0]:^50} | {header[1]:^25} | {header[2]:^25} | {header[3]:^25} | {header[4]:^25} | {header[5]:^25} | {header[6]:^25} | {header[7]:^25} | {header[8]:^25} | {header[9]:^25}|")
        f.write("\n|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~~|~~~~~~~~~~~~~~~~~~~~~~~~~~|")
        for x in range(len(inRTN[0])):
                  ...
                  if inRTN[3][x]<0:
                      space=""
                  f.write(f"\n|{inRTN[0][x]:^50} | {inRTN[1][x]:<25} | {inRTN[2][x]:<25} | {space+str(inRTN[3][x]):<25} | {inRTN[4][x]:<25} | {inRTN[5][x]:<25} | {inRTN[6][x]:<25} | {inRTN[7][x]:<25} | {inRTN[8][x]:<25} | {td:<25}|")
        ...
        while others.lower()=="yes":
            ...
            mutRO = float(input("Please enter its mutation rate (NOT LOG OF RATE) (ENTER -1 IF YOU DO NOT WANT A RATE): "))
            if mutRO!=-1:
                mutRO=math.log(mutRO)
            else:
                mutRO="No Mutation Rate"
            ...
            error=None
            if isinstance(mutRO,str):
                error=mutRO
            else:
                error=(mutRO-outO.item())
                out2[0].append(mutRO)
            ...
            f.write(f"\n|{nameO:^50} | {mutRO:<25} | {outO.item():<25} | {(error):<25} | {gSizeO:<25} | {aCountO/total:<25} | {gCountO/total:<25} | {cCountO/total:<25} | {tCountO/total:<25} | {mid:<25}|")
